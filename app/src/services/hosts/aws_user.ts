...
export class HostUserAws extends HostUserAwsBase implements HostUser {
    ...
    async _setup_displayer():Promise<void>{
        ...
        const files = await untar(await self.app_native.read_app_file('displayer.tar'))
    ...
    async _setup_lambda():Promise<void>{
        ...
        const fn_code = new Uint8Array(await self.app_native.read_app_file('responder_aws.zip'))
